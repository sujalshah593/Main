...
import { ArrowLeft, Activity, BookOpen, BrainCircuit } from 'lucide-react';

import ActivationLearningPanel from '../components/activation-functions/ActivationLearningPanel.jsx';
...
export default function ActivationFunctionsPage() {
  ...
  const [compareFunctions, setCompareFunctions] = useState(['sigmoid']);
  const [isCompareMode, setIsCompareMode] = useState(false);
  const [inputValue, setInputValue] = useState(0);
  const [params, setParams] = useState({ alpha: 0.1 }); // For Leaky ReLU and ELU
  ...
  const resetSimulator = () => {
    setInputValue(0);
    setParams({ alpha: 0.1 });
    setActiveFunction('sigmoid');
    setCompareFunctions(['sigmoid']);
      ...
      <div className={`grid grid-cols-1 ${isSidebarOpen ? 'xl:grid-cols-3' : 'xl:grid-cols-1'} gap-6 flex-1 h-full min-h-0 transition-all duration-300`}>
        ...
        <AnimatePresence>
          {isSidebarOpen && (
            ...
            >
              <ActivationLearningPanel activeFunction={activeFunction} />
            </motion.div>
          )}
        </AnimatePresence>
        ...
        <motion.div layout className={`${isSidebarOpen ? 'xl:col-span-2' : 'xl:col-span-1'} flex flex-col gap-6 overflow-y-auto pr-2 custom-scrollbar min-w-0`}>
          ...
          <ActivationControls
            ...
            setCompareFunctions={setCompareFunctions}
          ...
          <div className="glass-panel p-6 rounded-2xl border-t-2 border-t-lab-accent2/50 relative shadow-xl flex flex-col min-h-[400px]">
