...
import { ArrowLeft, LineChart, HelpCircle, Activity, BookOpen } from 'lucide-react';

import LearningPanel from '../components/function-plotter/LearningPanel.jsx';
...
export default function FunctionPlotterPage() {
  ...
  const [params, setParams] = useState({
    m: 1, c: 0, // linear: y = mx + c
    a: 1, b: 0, k: 0, // quadratic: y = ax^2 + bx + k, exponential: y = a*e^(bx) + k, log: y = a*ln(bx) + k
  });
  ...
  // For challenge mode
  const [challengeFunction, setChallengeFunction] = useState(null);
      ...
      <div className={`grid grid-cols-1 ${isSidebarOpen ? 'xl:grid-cols-3' : 'xl:grid-cols-1'} gap-6 flex-1 h-full min-h-0 transition-all duration-300`}>
        ...
        <motion.div layout className={`${isSidebarOpen ? 'xl:col-span-2' : 'xl:col-span-1'} flex flex-col gap-6 overflow-y-auto pr-2 custom-scrollbar min-w-0`}>
          ...
          <SimulatorControls
            ...
            setChallengeFunction={setChallengeFunction}
          />
