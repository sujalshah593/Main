...
export default function SpringQuiz() {
  ...
  const questions = [
    {
      id: 1,
      q: "According to Hooke's Law, extension is directly proportional to...",
      options: ["Mass", "Applied Force", "Gravity", "Elastic Limit"],
      correct: 1
    },
    {
      id: 2,
      q: "What does the slope of a Force vs Extension graph represent?",
      options: ["Elastic Limit", "Mass", "Spring Constant (k)", "Young's Modulus"],
      correct: 2
    },
    {
      id: 3,
      q: "What happens if you stretch a spring beyond its elastic limit?",
      options: ["It breaks immediately", "It returns to original length", "It becomes permanently deformed", "It becomes stiffer"],
      correct: 2
    }
  ];
    ...
    <div className="bg-slate-800 p-6 rounded-xl border border-slate-700 h-full">
      ...
      {!showResults ? (
        <button 
          ...
          className="w-full mt-8 py-3 bg-amber-500 hover:bg-amber-600 disabled:bg-slate-700 disabled:text-slate-500 text-white rounded-lg font-bold transition-all"
        >
          Submit Quiz
        </button>
      ) : (
